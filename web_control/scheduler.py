...
class ScheduleManager:
    def __init__(self, runtime_dir: Path, jobs: BuildJobManager, manager: HeadlessManager | None = None):
        ...
        # build & run lại mỗi repeat_hours. mode/daily_time/interval_hours
        # chỉ còn được đọc để tương thích với schedule.json cũ.
    ...
    def _load(self) -> None:
        ...
        # Giữ nguyên next_run_at đã lưu. Đây là điểm quan trọng khi reload
        # web/restart service: không biến lịch đã đặt thành lịch mới.
        if not self.enabled:
    ...
    def get_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "start_time": self.start_time,
            "repeat_hours": self.repeat_hours,
            # Trả alias cho client cũ trong giai đoạn chuyển đổi.
            "mode": "start_then_repeat",
            "daily_time": self.start_time,
            "interval_hours": self.repeat_hours,
            "worker_count": self.worker_count,
        ...
        while True:
            try:
                ...
                if self.enabled and self.next_run_at:
                    try:
                        target_dt = datetime.fromisoformat(self.next_run_at)
                        if target_dt.tzinfo is None:
                            target_dt = target_dt.replace(tzinfo=TZ_VN)
                        else:
                            target_dt = target_dt.astimezone(TZ_VN)
                    except Exception:
                        self._update_next_run()
                        self._save()
                        target_dt = None
    ...
    async def _trigger_scheduled_run(self, now: datetime | None = None) -> None:
        """Queue exactly one full Build & Run and schedule its next cycle."""
        ...
        async with self.transition_lock:
            if self.jobs.active_job() is not None:
                ...
                self.next_run_at = (now + timedelta(seconds=30)).isoformat(timespec="seconds")
                self._save()
                return

            logger.info("Đến giờ Build & Run! Ngắt toàn bộ Tà Thú và ưu tiên chạy lại NVHN...")
            try:
                if self.manager is not None:
