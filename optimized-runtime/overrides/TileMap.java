...
public final class TileMap {
    ...
    public static void gameAA() {
        if (OptimizedTuning.SKIP_DECORATIONS || HeadlessTuning.SKIP_DECORATIONS) return;
    ...
    public static void gameAE() {
        try {
            if (fieldCE[mapID] == null) {
                InputStream is = null;
                try {
                    is = "".getClass().getResourceAsStream("/map/" + mapID);
                    if (is != null) {
                        fieldCE[mapID] = new byte[is.available()];
                        is.read(fieldCE[mapID]);
                    } else {
                        fieldCE[mapID] = new byte[0];
                    }
                } catch (Exception e) {
                    fieldCE[mapID] = new byte[0];
                } finally {
