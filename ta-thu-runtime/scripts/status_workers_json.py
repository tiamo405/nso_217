#!/usr/bin/env python3
"""Return Tà Thú worker status as stable JSON for the web controller."""
...
def classify_pid(pid_file: Path, worker_dir: Path) -> tuple[int | None, str | None]:
    ...
    if "HeadlessMain" not in cmdline or "-Dnso.runtime=ta-thu" not in cmdline or str(worker_dir) not in cmdline:
        return None, "ERROR"
    return pid, "RUNNING"
...
def worker_status(worker_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "last_auto_log": last_matching_line(stdout_log, "AUTO TA THU"),
