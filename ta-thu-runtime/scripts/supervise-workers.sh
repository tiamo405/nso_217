...
stop_worker_dir() {
    ...
    if [[ "$cmdline" != *"HeadlessMain"* || "$cmdline" != *"-Dnso.runtime=ta-thu"* || "$cmdline" != *"$worker_dir"* ]]; then
        echo "Bỏ qua $worker_name: PID $pid không thuộc Tà Thú worker này." >&2
        return
    fi
...
find_repeated_status() {
    ...
    awk -v limit="$REPEATED_STATUS_LIMIT" '
        ...
        index($0, "AUTO TA THU PREP: đang tới Okaza") > 0 {
            prep = substr($0, index($0, "AUTO TA THU PREP:"))
            key = "prep " prep
            if (key == last_key) {
                repeated++
            } else {
...
restart_worker() {
    ...
    cmdline=$(tr '\0' ' ' <"/proc/$pid/cmdline" 2>/dev/null || true)
    if [[ "$cmdline" != *"HeadlessMain"* || "$cmdline" != *"-Dnso.runtime=ta-thu"* || "$cmdline" != *"$worker_dir"* ]]; then
        echo "Bỏ qua restart $worker_name: PID $pid không thuộc Tà Thú worker này." >&2
        return 1
    fi
...
if (( ${#worker_args[@]} == 0 )); then
    echo "Tà Thú supervisor đang chạy tất cả worker; Ctrl+C để dừng."
else
    echo "Tà Thú supervisor đang chạy worker: ${worker_args[*]}; Ctrl+C để dừng."
fi
...
while true; do
    ...
    for worker_dir in "${worker_dirs[@]}"; do
        ...
        if repeated_status=$(find_repeated_status "$worker_dir/stdout.log"); then
            echo "[$(date '+%F %T')] $worker_name có trạng thái AUTO TA THU bị lặp từ $REPEATED_STATUS_LIMIT lần liên tiếp; đang restart."
