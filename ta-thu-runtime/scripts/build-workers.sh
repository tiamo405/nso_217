...
if [[ -d "$WORKERS_DIR" ]]; then
    while IFS= read -r pid_file; do
        ...
        if [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null; then
            echo "Tà Thú worker PID $pid vẫn đang chạy. Hãy stop trước khi build lại." >&2
            exit 1
        fi
    done < <(find "$WORKERS_DIR" -mindepth 2 -maxdepth 2 -name bot.pid -type f)
fi

if [[ "$BUILD_TA_THU" != "0" ]]; then
    echo "Đang build Tà Thú classes..."
    TA_THU_ACCOUNT_CSV="$SOURCE_CSV" "$TA_THU_DIR/build-ta-thu.sh"
fi
...
STAGING_DIR=$(mktemp -d "$TA_THU_DIR/.workers-build.XXXXXX")
...
if [[ -d "$WORKERS_DIR" ]]; then
    backup_dir="$TA_THU_DIR/.workers-old.$$"
...
echo "Hoàn tất: $TOTAL tài khoản / $WORKER_COUNT Tà Thú worker tại $WORKERS_DIR"
