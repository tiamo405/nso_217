...
CLASSES_DIR=${TA_THU_CLASSES_DIR:-"$TA_THU_DIR/build/classes"}
...
if [[ ! -d "$CLASSES_DIR" ]]; then
    echo "Chưa có Tà Thú classes. Chạy ta-thu-runtime/scripts/build-workers.sh trước." >&2
    exit 1
fi
...
if (( ${#worker_args[@]} > 0 )); then
    ...
    if (( ${#worker_dirs[@]} == 0 )); then
        echo "Chưa có Tà Thú worker. Chạy ta-thu-runtime/scripts/build-workers.sh trước." >&2
        exit 1
    fi
fi
