...
final class MessageCollector implements Runnable {
    ...
    MessageCollector(Session_ME var1) {
        this.instance = var1;
        this.connectionGeneration = var1.connectionGeneration();
        this.input = var1.dis;
    }

    public final void run() {
        while (true) {
            if (!this.instance.currentConnection(this.connectionGeneration)) return;
            try {
                if (this.instance.gameAB()) {
                    MessageCollector var10 = this;
                    byte var2 = this.input.readByte();
                    if (!this.instance.currentConnection(this.connectionGeneration)) return;
                    ...
                    byte var5;
                    if (var2 == -32) {
                        var2 = this.input.readByte();
                        if (this.instance.getKeyComplete) {
                            var2 = Session_ME.gameAA(this.instance, var2);
                        }

                        var4 = Session_ME.gameAA(this.instance, this.input.readByte());
                        var5 = Session_ME.gameAA(this.instance, this.input.readByte());
                        byte var6 = Session_ME.gameAA(this.instance, this.input.readByte());
                        byte var7 = Session_ME.gameAA(this.instance, this.input.readByte());
                        var3 = (var4 & 255) << 24 | (var5 & 255) << 16 | (var6 & 255) << 8 | var7 & 255;
                    } else if (this.instance.getKeyComplete) {
                        var4 = this.input.readByte();
                        var5 = this.input.readByte();
                        var3 = (Session_ME.gameAA(this.instance, var4) & 255) << 8 | Session_ME.gameAA(this.instance, var5) & 255;
                    } else {
                        var3 = this.input.readUnsignedShort();
                    }
                    ...
                    while (var13 != -1 && var14 < var3) {
                        if ((var13 = var10.input.read(var12, var14, var3 - var14)) > 0) {
                            var14 += var13;
                    ...
                    try {
                        ...
                        BotMetrics.event("error", "packet_handler:" + var8.getClass().getName(), 1);
    ...
    private void gameAA(Message var1) {
        try {
            ...
            byte[] key = new byte[var2];

            int var3;
            for (var3 = 0; var3 < var2; ++var3) {
                key[var3] = var1.reader().readByte();
            }

            for (var3 = 0; var3 < key.length - 1; ++var3) {
                byte[] var10000 = key;
                var10000[var3 + 1] ^= key[var3];
            }

            this.instance.installKey(key, this.connectionGeneration);
